...
import { verifyPasswordResetCode, resendPasswordResetCode } from "../../redux/actions/userActions";
...
const PasswordResetVerification = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const getUserId = async () => {
            const storedUserId = await AsyncStorage.getItem('passwordResetUserId');
            if (storedUserId) {
    ...
    const handleVerify = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await dispatch(verifyPasswordResetCode(userId, verificationCode));
            
            if (response === 'success') {
                Toast.show({
                    type: "success",
                    text1: "Code Verified",
                    text2: "You can now set your new password",
                });
                navigation.navigate("resetpassword", { userId, verificationCode });
            } else {
                Toast.show({
                    type: "error",
    ...
    const handleResendCode = async () => {
        ...
        setIsResending(true);
        try {
            await dispatch(resendPasswordResetCode(userId));
            Toast.show({
                type: "success",
                text1: "Code Sent",
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="flex-1 bg-white p-5">
            <View className="flex-1">
                ...
                <View className="pt-5 flex-row items-center mb-8">
                    ...
                    <Text className="text-xl font-bold text-[#e01d47]">Verify Code</Text>
                </View>
                ...
                <View className="flex-1 justify-center items-center px-4">
                    ...
                    <View className="bg-[#fce8ec] p-6 rounded-full mb-6">
                        <Ionicons name="key-outline" size={60} color="#e01d47" />
                    </View>
