...
import { register, updateAvatar } from "../../redux/actions/userActions";
...
const SignUp = ({ navigation, route }) => {
    ...
    const disableBtn = !fname || !lname || !email || !password || !dateOfBirth || !phone || !address;
    ...
    const submitHandler = async () => {
        // Create the JSON object for registration
        ...
        if (avatar) {
            ...
            avatarFormData.append('upload_preset', 'ml_default'); // Replace with your preset
            ...
            try {
                ...
                // Show error toast for avatar upload failure
                Toast.show({
                    type: 'error',
        ...
        try {
            ...
            // Show error toast for registration error
            Toast.show({
                type: 'error',
                text1: 'Registration failed. Please try again.',
            });
    ...
    const openImagePicker = async () => {
        ...
        if (data.assets) {
            ...
            setAvatar(imageUri); // Set the new avatar preview
            setIsAvatarChanged(true); // Show update button
        }
    };
                            ...
                            <Text className="text-gray-700 ml-4">Date of Birth</Text>
                                            <TouchableOpacity onPress={() => setShowDatePicker(true)}>
                                                <TextInput
                                                    ...
                                                    value={dateOfBirth}
                                                    editable={false} // Make it non-editable, users will select from the picker
                                                    className="p-4 bg-gray-100 text-gray-700 rounded-2xl mb-3"
                                                />
                                            </TouchableOpacity>
                                            ...
                                            {showDatePicker && (
                                                <DateTimePicker
                                                    ...
                                                    value={new Date(dateOfBirth || Date.now())} // Default to current date if dateOfBirth is empty
                                                    mode="date"
                                                    is24Hour={true}
                                                    display="default"
