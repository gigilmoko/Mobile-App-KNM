...
import { verifyEmailCode, resendEmailVerificationCode } from "../../redux/actions/userActions";
...
const EmailVerification = ({ navigation }) => {
  ...
  const handleVerify = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await dispatch(verifyEmailCode(verificationCode));
    ...
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="flex-1 bg-white p-5">
      <View className="flex-1">
        ...
        <View className="pt-5 flex-row items-center mb-8">
          ...
          <Text className="text-xl font-bold text-[#e01d47]">Email Verification</Text>
        </View>
        ...
        <View className="flex-1 justify-center items-center px-4">
          ...
          <View className="bg-[#fce8ec] p-6 rounded-full mb-6">
            <Ionicons name="mail-outline" size={60} color="#e01d47" />
          </View>
          ...
          >
            <Text className="text-gray-600 text-center">
              Wrong email? <Text className="text-[#e01d47] font-semibold">Back to Sign Up</Text>
            </Text>
          </TouchableOpacity>
        </View>
      </View>
